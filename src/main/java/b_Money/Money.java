...
public class Money implements Comparable<Money> {
	...
	public String toString() {
		float amount = this.amount / 100f;
		Locale.setDefault(Locale.US);
		return String.format("%.2f %s", amount, currency.getName());
	}
	...
	public Money add(Money other) {
		int amount = (int) ((this.universalValue() + other.universalValue()) / currency.getRate());
		return new Money(amount, currency);
	}

	/**
	 * Subtracts Money from this Money, regardless of the Currency of the other Money.
	 * @param other The money that is being subtracted from this Money.
	 * @return New Money with the same Currency as this Money, representing the subtracted value.
	...
	public int compareTo(Money other) {
		return universalValue().compareTo((other.universalValue()));
	}
}